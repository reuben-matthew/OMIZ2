...
writer=pd.ExcelWriter(r'X:\OMIZ2\March_2018\Data_Cleaning\Python output\tables.xlsx',engine='openpyxl')
length=0
...
def openended_all (questionID):
    ...
    oa_exceldf.to_excel(writer,startrow=length+2,index=False)
    length+=len(oa_exceldf)
...
def openended_all_many(questionIDs):
    ...
    oam_exceldf.to_excel(writer, startrow=length+2, index=False)
    length+=len(oam_exceldf)
...
def openended_all_responses (questionIDs):
    ...
    oar_exceldf.to_excel(writer, startrow=length+2, index=False)
    length+=len(oar_exceldf)
...
def closeended_all (questionID):
    ...
    ca_exceldf.to_excel(writer, startrow=length+2, index=False)
    length+=len(ca_exceldf)
#closeended_all('A1') 
#closeended_all('A4')    
...
def closeended_all_many (questionIDs):
    ...
    cam_exceldf.to_excel(writer, startrow=length+2, index=False)
    length+=len(cam_exceldf)
...
def openended_acad (questionID):
    ...
    oacad_exceldf.to_excel(writer, startrow=length+2, index=False)
    length+=len(oacad_exceldf)
...
def openended_acad_many(questionIDs):
    ...
    oacadm_exceldf.to_excel(writer, startrow=length+2, index=False)
    length+=len(oacadm_exceldf)
...
def closeended_acad (questionID):
    ...
    cacad_exceldf.to_excel(writer, startrow=length+2, index=False)
    length+=len(cacad_exceldf)
...
def closeended_acad_many (questionIDs):
    ...
    cacadm_exceldf.to_excel(writer, startrow=length+2, index=False)
    length+=len(cacadm_exceldf)
